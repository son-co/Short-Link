    ...
    if (valid) {
      ...
      if (res1.data.success !== false) {
        ...
        if (res2.data.success === false) {
         ...
         // const res3 = await API.user.login({ username: addForm.username, password: addForm.password })password
          const res3 = await postData('http://localhost:8000/api/short-link/admin/v1/user/login','POST', { username: 'admin', password: 'admin123456' })
...
const login = (formEl) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      ...
      const res1 = await postData('http://8.222.220.102:8102/api/short-link/admin/v1/user/login','POST', { username: 'admin', password: 'admin123456' })
      console.log(res1)
      if (res1?.code === '0') {
